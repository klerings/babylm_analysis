...
if __name__ == "__main__":
    ...
    local = True if sys.argv[3] == "local" else False
    print(f"task: {task} num_examples: {num_examples} ({type(num_examples)}) local: {local} ({type(local)})")
    ...
    for subtask, examples in final_subtasks.items():
        ...
        with open(f"data/top_neurons/{task}/{model_path}_e{epoch}_{subtask}_top_neurons_per_subtask.pkl", "wb") as f:
