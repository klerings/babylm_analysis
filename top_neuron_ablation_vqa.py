...
def compute_metric_with_ablation(nnsight_model, examples, batch_size, top_neurons_mean, task, noimg):
    ...
    print(f"ablating the following neurons:")
    for l, edict in top_neurons_mean.items():
        print(l, edict.keys())
    for batch in tqdm(batches):
        ...
        if task == "blimp":
            ...
            def get_acc(nnsight_model):
                ...
                correct_sent_logits = torch.cat(correct_sent_logits, dim=0)
                incorrect_sent_logits = torch.cat(incorrect_sent_logits, dim=0)
                return (correct_sent_logits > incorrect_sent_logits).int()
...
def select_random_neurons(layers, neurons_per_layer, seed, num_selections=100):
    ...
    for _ in range(num_selections):
        ...
        neuron = random.randint(0, neurons_per_layer - 1)  # Random neuron between 0 and neurons_per_layer-1
...
def load_subtask_examples(subtask, lookup, tokenizer):
    ...
    for sample in tqdm(distractor_ds):
        ...
        clean_tokens = tokenizer(img_sample["question"], return_tensors="pt",padding=False)
        clean_prefix = clean_tokens.input_ids
        # add answer with preceding whitespace
        clean_answer = tokenizer(f" {img_sample['multiple_choice_answer']}", return_tensors="pt",
                                        padding=False).input_ids
        distractors = [tokenizer(f" {d}", return_tensors="pt",
                                        padding=False).input_ids for d in img_sample["distractors"]]
        ...
        pixel_values = img_processor(images=img_sample["image"].convert(mode="RGB"), return_tensors="pt")["pixel_values"]
...
if __name__ == "__main__":
    ...
    for subtask in subtasks:
        ...
        # get top neurons
        top_neurons_file = f"data/top_neurons/vqa/vqa_git_1vd125_s1_e29_nall/{subtask_dense}_top_neurons.pkl"
        top_neurons = load_subtask(top_neurons_file, top_k=100)

        top_neurons_file_noimg = f"data/top_neurons/vqa/vqa_git_1vd125_s1_e29_nall_noimg/{subtask_dense}_top_neurons.pkl"
        ...
        top_neurons_mean_noimg = get_mean_for_selected_neurons(top_neurons_noimg, mean_act_files)
        ...
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        with open(f"results/ablation_top_neurons/vqa/{prefix}/{subtask_dense}_top_neurons_ablated.pkl", "wb") as f:
