...
def load_blimp(task, n_samples=32, load_all=False):
    ...
    for i, sample in enumerate(full_dataset):
        ...
        final_samples[i]["prompts"] = [
            ("", f" {sample['sentence_good']}"),
            ("", f" {sample['sentence_bad']}"),
        ]
...
def load_vl_data(task="vqa", n_samples=32):
    ...
    for sample in tqdm(shuffled_distractors):
        if task == "vqa":
            ...
            if img_sample["image_id"] != sample["image_id"]:
                print(
                    f"Sample Mismatch: {sample['image_id']} - {img_sample['image_id']}"
                )
            else:
                img_sample["distractors"] = sample["distractors"]
                samples.append(img_sample)
        elif task == "winoground":
            # add image to json dataset samples because winoground batches
            # pairs together as one sample and json file splits them
    ...
    final_samples = {}
    # per sample create prompts consisting of tuple with
    # question-answer-prompt (context) and answer-options (continuation)
    for sample in samples:
        if task == "vqa":
            final_samples[sample["question_id"]] = sample
            final_samples[sample["question_id"]]["prompts"] = []
            final_samples[sample["question_id"]]["prompts"].append(
                transform_qa(sample["question"], sample["multiple_choice_answer"])
            )
            for d in sample["distractors"]:
                final_samples[sample["question_id"]]["prompts"].append(
                    transform_qa(sample["question"], d)
                )
        elif task == "winoground":
            final_samples[sample["id"]] = sample
            final_samples[sample["id"]]["prompts"] = [
                ("", f" {sample['caption_0']}"),
                ("", f" {sample['caption_1']}"),
            ]
...
def load_vqa_examples(tokenizer, img_processor, pad_to_length, n_samples):
    ...
    for sample_id, sample in samples.items():
        clean_tokens = tokenizer(sample["question"], return_tensors="pt", padding=False)
        clean_prefix = clean_tokens.input_ids
        # add answer with preceding whitespace
        clean_answer = tokenizer(
            f" {sample['multiple_choice_answer']}", return_tensors="pt", padding=False
        ).input_ids
        distractors = [
        ...
        pixel_values = img_processor(
            images=sample["image"].convert(mode="RGB"), return_tensors="pt"
        )["pixel_values"]
...
def load_ewok_examples(tokenizer, pad_to_length, n_samples):
    ...
    for i, sample_key in enumerate(sample_keys):
        ...
        text = (
            sample["Context1"] + " " + part1 + "\n" + sample["Context1"] + " " + part2
        )

        clean_prefix = tokenizer(text, return_tensors="pt", padding=False).input_ids

        tokens1 = tokenizer(
            part1, return_tensors="pt", padding=False
        ).input_ids.flatten()
        tokens2 = tokenizer(
            part2, return_tensors="pt", padding=False
        ).input_ids.flatten()
        context = tokenizer(
            sample["Context1"], return_tensors="pt", padding=False
        ).input_ids.flatten()
        ...
        answers_concat = torch.cat(
            (context, tokens1, context, tokens2, torch.tensor([eos_token_id]))
        )

        assert torch.equal(
            answers_concat, clean_prefix.flatten()
        ), "separate token encodings differ from combined token encoding"
...
def load_blimp_examples(tokenizer, pad_to_length, n_samples):
    ...
    for sample_id, sample in samples.items():
        if sample["one_prefix_method"] is True:
            ...
            if sent_good[:-1] == sent_bad[:-1]:
                ...
                sample["clean_prefix"] = sample["sentence_good"].replace(
                    sample["clean_answer"], ""
                )
    ...
    for sample in suitable_samples:
        clean_tokens = tokenizer(
            sample["clean_prefix"], return_tensors="pt", padding=False
        )
        clean_prefix = clean_tokens.input_ids
        clean_answer = tokenizer(
            sample["clean_answer"], return_tensors="pt", padding=False
        ).input_ids
        patch_answer = tokenizer(
            sample["patch_answer"], return_tensors="pt", padding=False
        ).input_ids
...
def load_git_model(model_dir, model_setting, epoch):
    ...
    spec = importlib.util.spec_from_file_location(
        "GitForCausalLM", f"{model_path}modeling_git.py"
    )
    ...
    )  # TODO: newly initialized for vision encoder: ['pooler.dense.bias', 'pooler.dense.weight']
